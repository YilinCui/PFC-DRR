...
{ 
    ...
    */
    ...
    {
        //NS_LOG_INFO ("\tFrom Node Id: " << tag.GetNodeId() << "\tPacket Timestamp: " << tag.GetTimestamp() << " delay="<< Now()-tag.GetTimestamp());
          NS_LOG_INFO ("\tFrom Node Id: " << tag.GetNodeId() << "\tPacket Timestamp: " << tag.GetTimestamp());
    }
...
CustomApplication::PromiscRx (Ptr<const Packet> packet, uint16_t channelFreq, WifiTxVector tx, MpduInfo mpdu, SignalNoiseDbm sn)
{
    ...
    */
    ...
    if (packet->PeekHeader (hdr))
    {
        ...
        {
            //record some information about neighbors
        }    
    }
}
